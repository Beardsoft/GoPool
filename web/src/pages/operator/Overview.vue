      ...
      <section data-section="activity" class="activity-section">
        ...
        <ul v-if="visibleEvents.length">
          <li v-for="event in visibleEvents" :key="event.id">
            ...
            <div><strong>{{ event.summary }}</strong><small>{{ event.source || event.category }}<template v-if="event.created_at"> · {{ new Date(event.created_at).toLocaleString() }}</template></small></div>
          </li>
        </ul>
...
.attention-section ul, .activity-section ul { display: grid; gap: 0; margin: 0; padding: 0; list-style: none; }.attention-section li, .activity-section li { display: flex; align-items: flex-start; gap: 14px; padding: 15px 0; border-top: 1px solid var(--app-border); }.event-severity, .activity-dot { width: 9px; height: 9px; margin-top: 6px; flex: 0 0 auto; border-radius: 50%; background: var(--nimiq-gold); }.event-severity[data-severity='error'], .activity-dot[data-severity='error'] { background: var(--nimiq-red); }.event-severity[data-severity='info'], .activity-dot[data-severity='info'] { background: var(--nimiq-light-blue); }.attention-section li div, .activity-section li div { display: grid; }.attention-section li small, .activity-section li small { margin-top: 4px; color: var(--app-faint); }
...
@media (max-width: 900px) { .metrics { grid-template-columns: 1fr 1fr; }.metrics article:nth-child(2) { border-right: 0; }.metrics article:nth-child(-n+2) { border-bottom: 1px solid var(--app-border); }.detail-grid { grid-template-columns: 1fr; } }
